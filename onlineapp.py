...
class MainWindow(CTk):
   def __init__(self):
       ...
       try:
           ...
           self.sock.connect(('0.tcp.eu.ngrok.io', 12136))
           hello = f"TEXT@{self.username}@[SYSTEM] {self.username} приєднався(лась) до чату!\n"
           self.sock.send(hello.encode('utf-8'))

           # ======= ДОДАНО: окремий потік для отримання повідомлень =========
   ...
   def toggle_show_menu(self):
       if self.is_show_menu:
           ...
           # ======= ДОДАНО: віджети бокового меню (ім'я + вибір теми) ======
   ...
   def show_menu(self):
       ...
       if not self.menu_frame.winfo_width() >= 200 and self.is_show_menu:
           ...
           # ======= ДОДАНО: видалення віджетів меню при закритті ===========
   ...
   def adaptive_ui(self):
       # ======= Адаптивне розташування всіх віджетів ===================
   ...
   # ======= ДОДАНО: відображення повідомлень =========================
   ...
   def send_message(self):
       # ======= ДОДАНО: збереження імені з поля меню ===================
